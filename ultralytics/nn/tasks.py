...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # --- WFU special handling ---
        if m is WFU:
            # WFU expects a list of input channels
            ...
            if verbose:
                LOGGER.info(f"{i:>3}{f!s:>20}{n_:>3}{module.np:10.0f}  {t:<45}{args!s:<30}")
            save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)
            layers.append(module)
            if i == 0:
                ch = []
            ch.append(c2[0] if isinstance(c2, list) else c2)
            continue
        # --- end WFU ---
